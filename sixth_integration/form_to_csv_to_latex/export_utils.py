...
def tables_to_csv(tables_instance, output_dir):
    ...
    all_formulas = tables_instance.get_all_formulas()
    ...
    for i, table in enumerate(all_tables, 1):
        ...
        # Export formulas to JSON (always create, even if empty)
        formula_filename = f"table{i}.json"
        formula_filepath = os.path.join(output_dir, formula_filename)
        
        table_formulas = all_formulas[i-1]  # Get formulas for this table (i-1 since enumerate starts at 1)
        with open(formula_filepath, 'w', encoding='utf-8') as f:
            json.dump(table_formulas, f, indent=2)
        created_files.append(formula_filepath)
